...
const T = {
  es: {
    ...
    'edu.ort.plan': 'Ver plan de carrera',
...
function applyLang(lang) {
  ...
  document.querySelectorAll('[data-i18n-list]').forEach(ul => {
    ...
    if (items) ul.innerHTML = items.map(text => `<li>${text}</li>`).join('');
  });
...
function ensureEducationLinks() {
  ...
  if (ortCard && !ortCard.querySelector('[data-auto-link="ort-plan"]')) {
    ...
    link.innerHTML = '<svg width="11" height="11" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" /><polyline points="14 2 14 8 20 8" /></svg><span data-i18n="edu.ort.plan"></span>';
  ...
  if (agileLink) agileLink.href = 'assets/cerif%20santander%20academy.pdf';
}
